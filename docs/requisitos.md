Tema escolhido: Gestor de Finanças Pessoais. 
1. Definição do Tema: 
Nome do Projeto:  
Descrição: A API permite que usuários organizem seus gastos e receitas pessoais, 
categorizando transações e acompanhando o saldo mensal. 
Usuários: 
● Usuário comum: Permite cadastrar suas receitas e despesas, visualizar relatórios e 
saldo. 
● Administrador(opcional): Pode gerenciar usuários e verificar estatísticas gerais 
(em caso de multiusuário). 
...
● RF04: O sistema deve permitir o cadastro de despesas (ex: alimentação, 
transporte). 
● RF05: O sistema deve permitir a categorização de transações (alimentação, lazer, 
moradia etc.). 
